...
class Application(tk.Tk):
    ...
    def process_row(self, row):
        ...
        if sae_j2012da_sheet not in self.sae_j2012da.sheet_names:
            self.summary_table.at[self.index, "Kommentar"] = "nicht vorhanden"
            self.summary_table.at[self.index, "Finding"] = "x"
            return

        if fault_code in self.sae_j2012da.parse(sae_j2012da_sheet).values and fault_code != "ISO/SAE Reserved" and fault_code != "Manufacturer Controlled DTC":
            self.summary_table.at[self.index, "Kommentar"] = "i.O."
            return

        if self.control_unit in self.manufacturer_dtc.sheet_names:
            manufacturer_dtc_sheet = self.control_unit
            manufacturer_dtc = self.manufacturer_dtc.parse(manufacturer_dtc_sheet).values
            if fault_code in manufacturer_dtc:
                self.summary_table.at[self.index, "Kommentar"] = "Unangemeldeter Manufacturer-specific DTC. Bitte prüfen."
                return

        self.summary_table.at[self.index, "Kommentar"] = "DTC ISO/SAE reserved: Aktuellen Kondiax Auszug prüfen."
    ...
    def process_summary_table(self):
        try:
            ...
            self.start_row = int(self.start_row_spinbox.get())
